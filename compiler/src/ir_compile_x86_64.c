...
static const char *get_reg_byte(enum Register reg) {
    switch (reg) {
        case REGNONE: _panic("Value is not in register");
        case RAX: return "al";
        case RBX: return "bl";
...
static enum Register call_reg(int n) {
    switch (n) {
        case 0: return RDI;
        case 1: return RSI;
        case 2: return RDX;
...
static const char *get_reg(enum Register reg, int size) {
    switch (size) {
...
static const char *get_qualifier(int size) {
    switch (size) {
        case 1: return "byte";
        case 8: return "qword";
...
static void occupy_reg(struct IRNode *node, enum Register reg) {
    if (node->spill) {
        node->reg = reg;
    }
}

static void from_reg(struct IRNode *node, int fd_text) {
    if (node->spill) {
...
static void free_reg(struct IRNode *node) {
    if (node->spill) {
        node->reg = REGNONE;
    }
    else if (node->node_type == IRNodeGlobal) {
...
static void ir_print_block(struct IRBlock *block, int fd_text) {
    _fputsi(fd_text, "._b", block->idx, "");
}

static void ir_print_value(struct IRNode *value, int fd_text) {
...
static void instr_prefix(const char *instr, int size, int fd_text) {
    _fputs3(fd_text, "    ", instr, " ");
    _fputs2(fd_text, get_qualifier(size), " ");
}

static void uninstr_v(const char *instr, struct IRNode *arg, int fd_text) {
    instr_prefix(instr, type_size(arg->type), fd_text);
    ir_print_value(arg, fd_text);
    _fputs(fd_text, "\n");
}

static void uninstr_r(const char *instr, enum Register arg, int size, int fd_text) {
...
static void bininstr_vv(const char *instr, struct IRNode *left, struct IRNode *right, int fd_text) {
    ...
    _fputs(fd_text, ", ");
    ir_print_value(right, fd_text);
    _fputs(fd_text, "\n");
}

static void bininstr_rv(const char *instr, enum Register left, struct IRNode *right, int fd_text) {
    instr_prefix(instr, type_size(right->type), fd_text);
    _fputs2(fd_text, get_reg(left, type_size(right->type)), ", ");
    ir_print_value(right, fd_text);
    _fputs(fd_text, "\n");
}

static void bininstr_vr(const char *instr, struct IRNode *left, enum Register right, int fd_text) {
    instr_prefix(instr, type_size(left->type), fd_text);
    ir_print_value(left, fd_text);
...
static void bininstr_vc(const char *instr, struct IRNode *left, const char *right, int fd_text) {
    instr_prefix(instr, type_size(left->type), fd_text);
    ir_print_value(left, fd_text);
...
static void ir_compile_phi(struct IRBlock *block, struct IRBlock *succ_block, int fd_text) {
...
static enum Register find_register(struct CodegenContext *context, struct IRNode *node) {
    long *reg_status = (void*)context->reg_status.ptr;
...
static void ir_compile_value(struct IRBuilder *builder, struct CodegenContext *context, struct IRBlock *block, struct IRNode *value, int fd_text) {
    enum IRNodeType type = value->node_type;

    if (type == IRNodeAlloca || type == IRNodePhi) return;
...
static void ir_compile_block(struct IRBuilder *builder, struct CodegenContext *context, struct IRBlock *block, int fd_text) {
    ...
    for (int k = 0; k < sz_insts; k++) {
        ...
        ir_compile_value(builder, context, block, value, fd_text);
        if (ir_value_is_terminator(value)) break;
    }
}

static void ir_compile_function(struct IRBuilder *builder, struct CodegenContext *context, struct IRFunction *function, int fd_text) {
    context->stack_phase = -32;
    ...
    for (int i = 0; i < cnt_values; i++) {
        ...
        enum Register reg = find_register(context, value);
        value->reg = reg;
        if (reg == REGNONE) {
            value->spill = true;
            value->stack_phase = context->stack_phase - type_size(value->type);
            context->stack_phase -= type_size(value->type);
        }
    ...
    for (int j = 0; j < sz_blocks; j++) {
        struct IRBlock *block = function->block_list.ptr[j];
        ir_compile_block(builder, context, block, fd_text);
    }

    vclear(&context->reg_status);
...
void ir_compile_x86_64(struct IRBuilder *builder, const char *filename_compile_output) {
    ...
    struct CodegenContext *context = (struct CodegenContext*)_malloc(sizeof(struct CodegenContext));
    context->reg_status = vnew();
    ...
    for (int i = 0; i < sz_functions; i++) {
        ...
        ir_compile_function(builder, context, function, fd_text);
    }
